...
class TestSplitNodesDelimiter(unittest.TestCase):
  ...
  def test_delim_bold_another(self):
  ...
  def test_extract_markdown_links(self):
    matches = extract_markdown_links(
      "This is is text with a [link](https://boot.dev) and [another link](https://wikipedia.org)"
    )
    self.assertListEqual(
      [
        ("link", "https://boot.dev"),
  ...
  def test_split_image(self):
    node = TextNode("This is text with an ![image](https://i.imgur.com/zjjcJKZ.png)",
    TextType.TEXT,
    )
